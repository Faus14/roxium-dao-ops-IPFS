...
export class IPFSService {
  ...
  async uploadFile(
    ...
    try {
      ...
      throw new Error(
        `Error al subir archivo a IPFS: ${error instanceof Error ? error.message : 'Error desconocido'}`
      );
    }
  }
   ...
   */
  async getFile(cidString: string): Promise<Buffer> {
    if (!this.initialized || !this.fs) {
      await this.initialize();
    }
    ...
    try {
      ...
      throw new Error(
        `Error al descargar archivo desde IPFS: ${error instanceof Error ? error.message : 'Error desconocido'}`
      );
    }
  }
